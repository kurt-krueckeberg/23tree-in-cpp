  ...
  public:
    ...
    class iterator_base : public std::iterator<std::forward_iterator_tag, std::pair<const Key,Value>> { // in order iterator
      ...
      protected:
         ...
         int   getChildIndex(const typename Tree::Node23 *p) const noexcept;
         void  getSuccessor() noexcept;

         const Node23 *getInternalNodeSuccessor(const typename Tree::Node23 *pnode) const noexcept;

         std::pair<const typename Tree::Node23 *, int>  getLeafNodeSuccessor(const typename Tree::Node23 *) const noexcept;
    ...
    class iterator : public iterator_base<tree23<Key, Value>> { // in order iterator
         ...
         bool operator==(const iterator& lhs) const { return iterator_base<tree23<Key, Value>>::operator==(static_cast< const iterator_base<tree23<Key, Value>>& >(lhs)); }
         bool operator!=(const iterator& lhs) const { return iterator_base<tree23<Key, Value>>::operator!=(static_cast< const iterator_base<tree23<Key, Value>>& >(lhs)); }
         
         iterator& operator++() { iterator_base<tree23<Key, Value>>::operator++(); return *this; }
    ...
    class const_iterator: public iterator_base<tree23<Key, Value>> { // in order iterator
      ...
      public:
         ...
         bool operator==(const const_iterator& lhs) const { return iterator_base<tree23<Key, Value>>::operator==(static_cast< const iterator_base<tree23<Key, Value>>& >(lhs)); }
         bool operator!=(const const_iterator& lhs) const { return iterator_base<tree23<Key, Value>>::operator!=(static_cast< const iterator_base<tree23<Key, Value>>& >(lhs)); }
         
         const_iterator& operator++() noexcept;
         const_iterator operator++(int) noexcept;
         const std::pair<const Key, const Value&> operator*() const noexcept; // KeyValue& is wrong. We don't want to change the key. How about std::pair<Key, Value&>?
         const std::pair<const Key, const Value&> *operator->() const noexcept { return &operator*(); } // KeyValue& or pair<Key, Value&>????
    };
...
/*
   ...
   2. or we could have the derived iterator class  cast away const as needed in its methods: const_cast<tree23<Key, Value>&>(tree) ?
         ...
         Node23 *getInternalNodeSuccessor(Node23 *pnode) const noexcept;

         std::pair<Node23 *, int>  getLeafNodeSuccessor(Node23 *) const noexcept;
 ...
 */
template<class Key, class Value> template<class Tree> inline tree23<Key, Value>::iterator_base<Tree>::iterator_base(Tree& lhs_tree, int x) : tree{lhs_tree}, current{nullptr},\
                                     key_index{0} 
{
}

template<class Key, class Value> template<class Tree>  tree23<Key, Value>::iterator_base<Tree>::iterator_base(iterator_base<Tree>&& lhs) : \
             tree{lhs.tree}, current{lhs.current}, key_index{lhs.key_index} 
{
   lhs.current = nullptr; // set to end
}

template<class Key, class Value> template<class Tree> inline bool tree23<Key, Value>::iterator_base<Tree>::operator==(const iterator_base<Tree>& lhs) const
{
 if (this == &lhs) {
     return true;
...
// ctor called by end()
/*
template<class Key, class Value> template<class Tree> inline tree23<Key, Value>::iterator_base<Tree>::iterator_base(const iterator_base<Tree>& lhs_tree, Node23 *ptr) : tree{lhs_tree}, current{nullptr}
{
}
*/
/*
...
template<class Key, class Value> template<class Tree> inline int tree23<Key, Value>::iterator_base<Tree>::getChildIndex(const typename Tree::Node23 *p) const noexcept
{
...
template<class Key, class Value> template<class Tree> std::pair<const typename Tree::Node23 *, int> tree23<Key, Value>::iterator_base<Tree>::getLeafNodeSuccessor(const typename Tree::Node23 *pnode) const noexcept
{
  ...
  switch (child_index) {
   ...
   /* 
    ...
    2nd slot. If it was in the first slot, getPredessor() handles this). In a 2 3 tree the middle child is a "left child" of the 2nd key of a 3-node parent.
    The logic to find the first "left" child pointer is identical to the case when the leaf node is the third child of a 3-node (and this third child is a
    2-node or a 3-node with the key in the second position). 


 logic now involves two use cases. Either the leaf node is

1. the right child  of a 2-node, which is "case 1:" above. 

   pnode->parent->isTwoNode() == true && pnode == pnode->parent->children[1]

2. the right child of a three node

   pnode->parent->isThreeNode() == true && pnode == pnode->parent->children[2] && suc_key_index == 1

Note: If the leaf node itself is a 3-node, the suc_key_index will be 1 (if it was 0, getPredecessor() handled it already).

Finding the successor in this situation is discussed in http://ee.usc.edu/~redekopp/cs104/slides/L19_BalancedBST_23.pdf slide #9. 

In both a binary tree and a 2 3 tree...

If a right child exists, the successor is the left most node of the right subtree. 
...
parent. That parent parent is the successor of pnode.

If you get to the root w/o finding a node that is a left child, there is no successor.

Note: In a 2 3 tree, the 2nd key of a 3-node is greater than all values in the subtree rooted at its middle child. Thus, when viewed from its 2nd key, the 
middle child pointer is a "left" subtree pointer ofr the 2nd key. When viewed from the 1st key, the middle child is right subtree pointer.

In the tree below, if [50] is the pnode leaf node, the successor of [50] is the second key of the 3-node [17, 60]. When we walk up the parent chain from [50], the
first left child pointer we encounter is the middle child of the 3-node [17, 60], which is the "left" child of 60. So 60 is the next largest key, the successor, of
[50].
...
So the remaining logic, which applies to both 2 and 3-node leaves, requires finding the first ancestor that is a "left" child pointer somewhere up the ancestral trail from pnode but
 ...
 */
template<class Key, class Value> template<class Tree> std::pair<const typename Tree::Node23 *, int> tree23<Key, Value>::iterator_base<Tree>::findLeftChildAncestor() noexcept
{
...
template<class Key, class Value> template<class Tree> void tree23<Key, Value>::iterator_base<Tree>::getPredecessor() noexcept	    
{
  // TODO: implement
}

template<class Key, class Value> const typename std::pair<const Key, Value&> tree23<Key, Value>::iterator::operator*() const noexcept	    
{
  return std::make_pair(iterator_base<tree23<Key, Value>>::current->keys_values[0].key, iterator_base<tree23<Key, Value>>::current->keys_values[0].value);
}
...
{
   iterator_base<tree23<Key, Value>>::getSuccessor(); // call base class method that sets current and key_index 
}
...
template<class Key, class Value> typename tree23<Key, Value>::const_iterator tree23<Key, Value>::const_iterator::operator++(int) noexcept	    
{
 ...
 iterator_base<tree23<Key, Value>>::getSuccessor(); 

 return *this;
}

template<class Key, class Value> const typename std::pair<const Key, const Value&> tree23<Key, Value>::const_iterator::operator*() const noexcept	    
{
  return std::make_pair(iterator_base<tree23<Key, Value>>::current->keys_values[0].key, iterator_base<tree23<Key, Value>>::current->keys_values[0].value);
}
