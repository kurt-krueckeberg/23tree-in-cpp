...
template<class Key, class Value> class tree23 {
  ...
  public:
    ...
    class iterator : public std::iterator<std::forward_iterator_tag, Value> { // in order iterator
         ...
         /*
         std::shared_ptr< std::deque<int> >  pstack; // TODO: Question: This is not really reusable between copied instances because you have to pop and push
                                                     // onto the stack as you move to the next key--right;
          */
         // child_index can be calculated, so let's fist try an approach base solely on calculating the child index. 
